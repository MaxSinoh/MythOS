...
void map_page(VirtualMemoryManager* vmm, uint64_t virt_addr, uint64_t phys_addr, uint64_t flags) {
    if (!vmm || !vmm->pml4 || !vmm->pmm) return;
...
uint64_t allocate_and_map_pages(VirtualMemoryManager* vmm, uint64_t virt_addr, uint64_t count, uint64_t flags) {
    if (!vmm || !vmm->pmm) return 0;
    
    // 记录分配的物理地址，用于回滚
    uint64_t *phys_addrs = (uint64_t*)malloc(count * sizeof(uint64_t));
    if (!phys_addrs) return 0;
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        if (!phys_addr) {
            ...
            for (uint64_t j = 0; j < i; j++) {
                ...
                pmmFreePage(vmm->pmm, phys_addrs[j]);
            }
            free(phys_addrs);
            return 0;
        }
