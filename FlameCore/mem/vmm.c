...
static PageTable* createPageTable()
{
    ...
    PageTable* page_table = (PageTable*)phys_addr;  // 将物理页地址转换为虚拟地址
...
void initVMM()
{
    ...
    {
        map((void*)addr, (void*)addr, 0x03); // 映射所有物理地址到虚拟地址，权限为可读可写
    }
...
int map(void* virtual_addr, void* physical_addr, uint64_t flags)
{
    ...
    if (!pdptEntry->present)
    {
        ...
        pdptEntry->write = (flags & 0x2) ? 1 : 0;     // 设置PDT表
...
void* vtop(void* virtual_addr)
{
    ...
    return (void*)(uintptr_t)(pteEntry->phys_addr << 12); // 返回物理地址
    // 物理地址 = 页框地址 + 页内偏移
